...
Assert(!offlineDecision.IsAllowed, "슬롯맵 검증 전에는 오프라인 이송도 차단되어야 합니다.");

var slotVerified = simulator.VerifySlotMap();
var slotReadyDecision = commandGate.Evaluate(offlineCommand, slotVerified, approvedTeachingLoaded: false);
Assert(slotReadyDecision.IsAllowed, "슬롯맵 검증 후에는 오프라인 검증 명령이 허용되어야 합니다.");
...
auditLog.Append(slotReadyDecision);
...
var rotateToSource = simulator.AdvanceCycle("FOUP A -> CHAMBER A");
Assert(rotateToSource.SequenceProgress == 12, "첫 스텝은 HOME에서 FOUP 방향으로 회전 정렬이어야 합니다.");

var extendToSource = simulator.AdvanceCycle("FOUP A -> CHAMBER A");
Assert(extendToSource.SequenceProgress == 25, "두 번째 스텝에서만 FOUP 방향으로 블레이드가 전진해야 합니다.");
